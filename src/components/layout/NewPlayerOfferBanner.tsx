...
const LOCAL_STORAGE_KEY = 'hasDismissedNewPlayerOffer_v1'; // Added a version to reset for existing users if needed
...
export default function NewPlayerOfferBanner() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Only run localStorage logic on the client side after mount
  ...
  return (
    <div
      className={cn(
        ...
        "relative flex items-center justify-center text-center w-full z-20" 
      )}
    >
      <div
        className={cn(
